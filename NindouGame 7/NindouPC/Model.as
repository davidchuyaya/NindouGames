package
{
	...
	public class Model extends EventDispatcher
	{
		...
		//booleans
		private var _stopPlayer:Boolean = false;
		//map stuff
		...
		{
			//write the read from file functions later
		}
		public function update():void
		{
			...
			{
				_playerPosition += _playerPositionChange;
				_traveledLength += 1;
				//increase the y axis speed
				increaseSpeed();
				//add score depending on how far you've traveled
				if (_traveledLength % 10 == 0)
				{
					_score += 1;
				}
				/*
				* PLAYER POSITION STUFFS
				*/
				//make the player's x axis speed decrease if it should
				if (_stopPlayer)
				{
					_playerPositionChange *= 0.7;
				}
				//stop the player if his speed is near zero
				if (_playerPositionChange < 0.5 && _playerPositionChange > -0.5 && _stopPlayer)
				{
					resetPlayerPositionChange();
					_stopPlayer = false;
				}
		...
		//increase the player's falling speed
		...
		*/
		public function goLeft(left:Boolean):void
		{
			if (left)
			{
				_playerPositionChange = - scooch;
			}
			else
			{
				_playerPositionChange = scooch;
		...
		*/
		...
		public function get playerPositionChange():int
		{
			return _playerPositionChange;
		}
		public function set stopPlayer(yeaOrNay:Boolean):void
		{
			_stopPlayer = yeaOrNay;
		}
